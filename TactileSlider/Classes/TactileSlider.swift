...
@IBDesignable open class TactileSlider: UIControl {
	...
	@objc func didPan(sender: UIPanGestureRecognizer) {
        ...
        var valueChange = valueChangeForTranslation(translation)
        
        var minMaxReached = false
        if value == minimum && valueChange < 0 || value == maximum && valueChange > 0 {
            minMaxReached = true
            valueChange = 0
        }
        ...
        // It's different if we are out of range
        let outOfRange = (newValue != value)
        ...
        if isContinuous || sender.state == .ended || sender.state == .cancelled || outOfRange || minMaxReached {
            sendActions(for: .valueChanged)
        }
        
        if (sender.state != .ended && sender.state != .cancelled && sender.state != .failed) || outOfRange || minMaxReached {
